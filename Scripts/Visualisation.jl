...
bubblePlot = plot(Time.(asks.DateTime), asks.Price, seriestype = :scatter, marker = (:red, stroke(:red), 10 * (asks.Volume / maximum(asks.Volume)), 0.7), label = "Ask (LO)", xlabel = "Time", ylabel = "Price (ticks)", legend = :topleft, legendfontsize = 5, xrotation = 30)
plot!(bubblePlot, Time.(bids.DateTime), bids.Price, seriestype = :scatter, marker = (:blue, stroke(:blue), 10 * (bids.Volume / maximum(bids.Volume)), 0.7), label = "Bid (LO)")
plot!(bubblePlot, Time.(sells.DateTime), sells.Price, seriestype = :scatter, marker = (:red, stroke(:red), :utriangle, 10 * (sells.Volume / maximum(sells.Volume)), 0.7), label = "Sell (MO)")
plot!(bubblePlot, Time.(buys.DateTime), buys.Price, seriestype = :scatter, marker = (:blue, stroke(:blue), :utriangle, 10 * (buys.Volume / maximum(buys.Volume)), 0.7), label = "Buy (MO)")

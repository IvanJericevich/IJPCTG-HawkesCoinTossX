...
function LogReturnDistribution(resolution::Symbol; lobFile::String, cummulative::Bool = false)
    ...
    if cummulative
        ...
        plot!(empiricalDistribution, theoreticalDistribution, linecolor = :green, label = "Fitted Normal")
        ...
        savefig(empiricalDistribution, string("Figures/Log-Return Distribution - ", resolution, ".pdf"))
    end
end
#---------------------------------------------------------------------------------------------------

#----- Log-return and absolute log-return autocorrelation -----#
function LogReturnAutocorrelation(lag::Int64, lobFile::String)
    # Obtain log-returns
    ...
    savefig(autoCorrPlot, "Figures/Log-Return Autocorrelation.pdf")
end
#---------------------------------------------------------------------------------------------------

#----- Trade sign autocorrealtion -----#
function TradeSignAutocorrelation(lag::Int64, lobFile::String)
    # Extract trade signs
    data = CSV.File(string("Data/", lobFile, ".csv"), missingstring = "missing") |> DataFrame
    ...
    savefig(autoCorrPlot, "Figures/Trade-Sign Autocorrelation.pdf")
end
#---------------------------------------------------------------------------------------------------

#----- Trade inter-arrival time distribution -----#
function InterArrivalTimeDistribution(lobFile::String)
...
function ExtremeLogReturnPercentileDistribution(resolution::Symbol, side::Symbol; lobFile::String)
    # Obtain log-returns of price series
    ...
    # Plot
    extremePercentileDistributionPlot = plot(observations, normalize = :pdf, linecolor = :blue, xlabel = string("Log return Extreme", side, " percentiles"), ylabel = "Density", annotations = (3, y[3], Plots.text(string("α=" α), :left)), legend = false)
    ...
    savefig(extremePercentileDistributionPlot, string("Figures/Extreme", side, " Log-Return Percentiles Distribution - ", resolution,".pdf"))
end
#---------------------------------------------------------------------------------------------------

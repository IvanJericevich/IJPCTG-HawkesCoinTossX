...
function Validation(model::Int64; dimension::Int64 = 10, format = "pdf")
    ...
    observations = sort(integratedIntensities[1])
    quantiles = map(i -> quantile(Exponential(1), i / length(integratedIntensities[1])), 1:length(integratedIntensities[1]))
	qqPlot = plot([quantiles quantiles], [observations quantiles], seriestype = [:scatter :line], xlabel = "Exponential theoretical quantiles", ylabel = "Sample quantiles", label = [labels[1] ""], marker = (4, colors[1], stroke(colors[1]), 0.7), linecolor = :black, legendfontsize = 5, legend = :topleft, xlim = (0, 9), ylim = (0, 9))
	LBTemp = LjungBoxTest(integratedIntensities[1], 100, 1) # Ljung-Box - H_0 = independent
	...
	for m in 2:dimension
		observations = sort(integratedIntensities[m])
	    quantiles = map(i -> quantile(Exponential(1), i / length(integratedIntensities[m])), 1:length(integratedIntensities[m]))
		plot!(qqPlot, [quantiles quantiles], [observations quantiles], seriestype = [:scatter :line], label = [labels[m] ""], marker = (4, colors[m], stroke(colors[m]), 0.7), linecolor = :black)
		LBTemp = LjungBoxTest(integratedIntensities[m], 100, 1) # Ljung-Box - H_0 = independent
